-- Only schema change allowed: add bank_accounts.branch_name.
-- Keep all other mobile columns/constraints as-is.
-- Fix admin_create_customer inserts to use:
--   nominees: nominee_name, relationship, nominee_aadhaar
--   bank_accounts: account_holder_name (required) + branch_name

ALTER TABLE public.bank_accounts
  ADD COLUMN IF NOT EXISTS branch_name TEXT;

-- Undo prior admin migrations that relaxed holder nullability (034).
UPDATE public.bank_accounts
SET account_holder_name = COALESCE(NULLIF(trim(account_holder_name), ''), bank_name, 'Unknown')
...
BEGIN
  ...
  -- Live schema (mobile): nominee_name + relationship + nominee_aadhaar
